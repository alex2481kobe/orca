...
// Use the returned config VERBATIM — no overrides. Anything we substitute here is
// a field this proof stops proving, and the base URL in particular used to be
// silently replaced, so a missing or wrong one would have passed.
const advertisedBase = String(leaseEnv.ORCA_AGENT_TOOLS_BASE_URL || '');
step('tokenized: the config advertises the port the daemon actually bound',
  advertisedBase.replace(/\/$/, '') === base2, advertisedBase || '(missing)');
