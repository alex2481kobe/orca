...
const CORE_V2_TOOLS = {
  ...
  'orchestrator.status': 'orchestrator__status',
  ...
  'audit.queue_one': 'audit__queue_one',
...
test('TOOL_DEFINITIONS exposes exactly the current tool set including the core v2 tools', () => {
  // Snapshot the total surface so adding OR removing a tool is a deliberate,
...
test('MCP tools/list advertises the core v2 tools to an orchestrator with underscored names', async () => {
  ...
  assert.equal(orchestratorCallable, 28);
